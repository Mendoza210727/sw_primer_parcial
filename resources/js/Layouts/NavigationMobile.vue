<template>
  ...
  <transition enter-active-class="transition ease-in-out duration-150"
    ...
    <aside v-show="$page.props.showingMobileMenu"
      class="fixed inset-y-0 z-20 flex-shrink-0 w-64 mt-16 overflow-y-auto bg-gradient-to-r from-blue-500 to-blue-700 text-white md:hidden">
      <div class="py-4">
        ...
        <ul class="mt-6">
          <li class="relative px-6 py-3">
            <ResponsiveNavLink :href="route('dashboard')" :active="route().current('dashboard')" class="flex items-center hover:bg-blue-600">
              <template #icon>
          ...
          <li class="relative px-6 py-3">
            <ResponsiveNavLink :href="route('projects.index')" :active="route().current('projects.index')" class="flex items-center hover:bg-blue-600">
              <template #icon>
                <svg class="w-5 h-5" aria-hidden="true" fill="none" stroke-linecap="round" stroke-linejoin="round"
                  stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
                  <path
          ...
          <li class="relative px-6 py-3">
            <ResponsiveNavLink :href="route('users.index')" :active="route().current('users.index')" class="flex items-center hover:bg-blue-600">
              <template #icon>
          ...
          <li class="relative px-6 py-3">
            <ResponsiveNavLink :href="route('about')" :active="route().current('about')" class="flex items-center hover:bg-blue-600">
              <template #icon>
                <svg class="w-5 h-5" aria-hidden="true" fill="none" stroke-linecap="round" stroke-linejoin="round"
                  stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
                  <path
